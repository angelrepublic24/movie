<?php
  if (isset($_GET{'v'}))
  {
    ...
    $sql = "DELETE FROM `post` WHERE codigo = '$veli'";
    $result=$mysqli->query($sql) or trigger_error($mysqli->error." [$sql]");
    echo "<script language=\"javascript\">
          window.location.href=\"?p=post\";
        </script>";
  }
  ...
  <script>
      function showDetailsScroll(){
          ...
          $.ajax({
                  ...
                  url:   'paginas/post-data.php', //archivo que recibe la peticion
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Listo de post</h3>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <section>
                ...
                if ($result->num_rows > 0) {
                  ...
                  while($row =$result->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>
                              <a href='?p=post-v&v=".utf8_decode($row['codigo'])."'>".$row['titulo']."
                              </a>
                            </td>
                            <td class='d-flex justify-content-center '>
                              <a class='btn btn-danger text-mobile align-self-end' href='?p=post&v=".utf8_decode($row['codigo'])."'><i class='fa fa-trash ' aria-hidden='true'></i>
                              </a>
                            </td>
                        </tr>";
                  }
